...
export async function appRoutes(app: FastifyInstance){
  ...
  app.get('/summary',async () => {
    // [ {date:19/01, amount: 5, completed:1},{date:20/01,amount:2,completed:0} ] 
    const summary = await prisma.$queryRaw`
      SELECT 
        D.id,
        D.date,
        (SELECT cast(count(1) as float) FROM day_habits DH WHERE DH.day_id = D.id) as completed,
        (
          SELECT 
